...
CLIENT_CONFIG = {
  "web": {
	...
	'redirect_uris': os.environ.get('GOOGLE_REDIRECT_URIS', None),
...
def ad_campaign_create(request):
    try:
        ...
        if age_range:
            for i in age_range:
                criterions.append(
                    age_range_criterion(client, i, campaign_response.results[0].resource_name, 'campaign'))
        if criterions:
...
def call_client(request):
    ...
    try:
        credentialsSession = request.session.get('credentials')
        if credentialsSession:
            credentials = credentialsSession
        else:
            credentials = {
                'developer_token': os.environ.get('GOOGLE_DEVELOPER_TOKEN', None),
                'use_proto_plus': True,
            }
        user = Authorizations.objects.get(user=user, ad_platform='google_ads')
...
def credentials_to_dict(credentials, request):
    try:
...
def disable(request):
    try:
        user = User.objects.get(pk=1)
        Authorizations.objects.filter(user=user, ad_platform='google_ads').delete()
        return JsonResponse({'message':'Google Ads Disabled Succesfully!'})
    except Exception as e:
        print(e)
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
...
def enable(request):
    try:
        # print(CLIENT_CONFIG['web']['client_id'])
        user = User.objects.get(pk=1)
        ...
        flow.redirect_uri = 'http://' + env('SERVER', default='127.0.0.1:8000') + '/google_ads/authorize'
...
def oauth(request):
    try:
        ...
        request.session['credentials'] = credentials_to_dict(credentials, request)
        ...
        if len(account_ids) == 1:
            ...
            authorization.ip_address = get_client_ip(request)
            authorization.save()
            request.session['account_id'] = account_id
            print("account id in session", request.session['account_id'])
            # request.session['credentials']['login_customer_id']=account_id
            ...
            print(accounts)
            print('ACCOUNTS!!!')
            return render(request, 'home/choose_account.html', {'accounts': accounts, 'ad_platform': 'google_ads'})
...
def authorize(request):
    try:
        ...
        flow.redirect_uri = 'http://127.0.0.1:8000/google_ads/authorize'
        ...
        try:
            ...
            if len(account_ids) == 1:
                ...
                authorization.save()
                request.session['account_id'] = account_id
                print("account id in session", account_ids)

            customerIds = extractAccountIds(account_ids)

        except InvalidGrantError as ex:
            print('Authentication has failed: %s' % ex)

        return redirect(f"http://localhost:3000/linkclicks/app/verify?accountIds={customerIds}")
...
@csrf_exempt
def saveAccountID(request):
    try:
        context = {}
        if request.method == "POST":
            body_unicode = request.body.decode('utf-8')
            body_data = json.loads(body_unicode)
            accountID = body_data['accountID']
            ad_platform = body_data['ad_platform']

            user = User.objects.get(pk=1)
            authorization = Authorizations.objects.filter(user=user, account_id=None,
                                                          ad_platform=ad_platform).update(
                account_id=accountID, account_name=accountID, date_time=datetime.utcnow(), )

        authorized_platforms_list = list(
            Authorizations.objects.filter(user=user).exclude(account_id__isnull=True).all())
    ...
    return JsonResponse({'authorization' : "successfull added"})

def authorizedPlatforms(request):
    user = User.objects.get(pk=1)
    authorized_platforms_list = Authorizations.objects.filter(user=user).exclude(account_id__isnull=True).all()
    data = serialize("json", authorized_platforms_list)
    data = json.loads(data)
    return JsonResponse(data, safe=False)

def extractAccountIds(account_ids):
    customerIds = []
    for item in account_ids:
        customerIds.append(int(item[10:]))
    return customerIds