...
def ad_campaign_create(request):
	try:
		...
		if age_range:
			for i in age_range:
				criterions.append(age_range_criterion(client,i, campaign_response.results[0].resource_name,'campaign'))
		if criterions:
...
def call_client(request):
	...
	try:
		if 'credentials' in request.session:
			...
			credentials={'developer_token':  os.environ.get("GOOGLE_DEVELOPER_TOKEN", None), 'use_proto_plus': True}
		user=Authorizations.objects.get(user=request.user,ad_platform='google_ads')
...
def disable(request):
	try:
		Authorizations.objects.filter(user=request.user, ad_platform='google_ads').delete()
...
def enable(request):
	try:   
		...
		flow.redirect_uri = 'https://' + env('SERVER', default='127.0.0.1') + '/google_ads/oauth'
